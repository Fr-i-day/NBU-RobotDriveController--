...
void chassis_move(float target_speed, float target_dir, float target_omega); // 直线

typedef struct
{
    double angle;  // 底盘当前角度（弧度）
    double Radius; // 底盘旋转半径（底盘中心到轮子的长度）

} Chassis;
extern Chassis chassis;
extern int speed_out[4];
#endif
